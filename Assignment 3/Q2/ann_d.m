...
theta1 = 0.0001*randn(units, 127);
theta2 = 0.0001*randn(3,units+1);
...
while (true)
    ...
    for i = 1:m
        ...
        a = [1 ; softplus(theta1*x(i,:)')]; % 101*1 (oj for 2nd layer)
        o = softplus(theta2*a); % 3*1
        
        z = -(y(i,:)' - o) .* sigmoid(o); % 3*1
        gradient2 = z * a'; % (3*1) * (1*101)
        
        % 100*127 =  ( (1*3) * (3*100) )   .*  ( 100*1 )
        gradient1 = (z' * theta2(:,2:units+1))' .* sigmoid(a(2:units+1)) ; % 100*1
